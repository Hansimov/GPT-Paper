...
class PDFTextBlockCategorizer:
    def __init__(self, doc_blocks, blocks_format="dict"):
        self.doc_blocks = doc_blocks
        self.blocks_format = blocks_format

    def generate_categorize_vectors(self):
        if self.blocks_format == "dict":
            categorize_metrics = []
    ...
    def get_most_common_category(self):
        if self.blocks_format == "dict":
            ...
            logger.info(
                f"{len_filtered_doc_blocks} blocks remained of {len_doc_blocks}."
            )
